# 學習系統

一套以能力養成為核心、適合手機使用、可自由替換學習主題的開源系統。

「AI 顧問養成」是第一個主題範本，目標是讓學習者能判斷需求、設計方案、診斷問題，並交付有價值的智慧代理工作流程。

## 目前功能

- 以 Fold 折疊手機為優先的漸進式網頁應用程式（PWA）
- 只對必要知識設定前置條件的主線任務
- 支線任務、複習清單與顧問情境挑戰
- 經驗值、等級、學習時間與活動月曆
- 本機 PDF 文字擷取、任務頁碼範圍閱讀與精準跳頁
- 可支援其他學習領域的主題範本格式
- GitHub Pages 自動發布流程

## 使用方式

部署後開啟 GitHub Pages 網址，在 Fold 的瀏覽器選擇「加到主畫面」，即可用接近 App 的方式開啟。第一次閱讀時選擇本人合法持有的 PDF，後續任務會直接顯示指定頁碼的課程文字，也能一鍵切到正確 PDF 頁面。

## 專案結構

```text
.
├── index.html                 # 學習系統主畫面
├── styles.css                 # Fold 優先的視覺樣式
├── app.js                     # 本機學習引擎與互動
├── pdf-reader.js              # PDF 文字擷取、精準跳頁與 IndexedDB
├── vendor/pdfjs/              # 固定版本的 Mozilla PDF.js
├── templates/
│   └── ai-consultant/         # 第一個學習主題範本
├── docs/                      # 產品決策與製作指南
└── .github/workflows/         # GitHub Pages 發布流程
```

## 語言規則

使用者可見內容與學習內容預設使用台灣繁體中文。英文原文只在教材本身採用「中文名詞（English Term）」格式時保留；系統不得自行翻譯、補寫或擴充英文術語。

## 隱私設計

學習日誌、經驗值與學習時段保留在裝置的瀏覽器儲存空間。使用者選擇的 PDF 與擷取文字保留在 IndexedDB，不會上傳到 GitHub 或第三方服務。公開 Repository 只包含可重複使用的學習範本、PDF.js 程式庫與示例資料，不包含個人紀錄或受版權保護的教材。

## 建立新的學習專案

複製 `templates/ai-consultant`，再替換名稱、能力、任務與閱讀索引。學習介面、進度系統、月曆與複習機制都可沿用。

詳細規則請參閱[專案章程](docs/PROJECT_CHARTER.md)、[產品與使用規格](docs/PRODUCT_SPEC.md)、[QA 回歸清單](docs/QA_CHECKLIST.md)、[範本製作指南](docs/TEMPLATE_AUTHORING.md)與[語言內容規範](docs/LANGUAGE_POLICY.md)。
